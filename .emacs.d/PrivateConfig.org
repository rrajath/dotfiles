...
* Introduction
This config is built by copying parts from [[https://github.com/daviwil/dotfiles/blob/master/Emacs.org][System Crafters dotfiles repo]], what I found on the internet (Reddit, Discord, Stack Overflow, etc.) and some of my own config. Some *bold*, some /italics/, let's see how this goes!
...
** TODO Why literate config?
* Current Issues
** DONE Warning message on startup
CLOSED: [2021-09-01 Wed 21:05]
This message shows up at Emacs startup for some reason:
#+begin_src
Warning (package): Unnecessary call to ‘package-initialize’ in init file
#+end_src
** TODO Capture templates point to default org directory
Instead of my custom org directory. Pretty sure this is happening because of out-of-order variables setup in my org config
